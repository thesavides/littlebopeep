  ...
  return (
    <div className="fixed inset-0 bg-slate-900 z-50 flex flex-col">
      ...
      <div className="bg-slate-800 px-4 py-3 flex items-center gap-3">
        <button onClick={onCancel} className="text-slate-400 hover:text-white text-xl leading-none">
          ←
        </button>
        <div className="flex-1">
          <p className="text-white font-semibold text-sm">📴 Offline Capture</p>
          <p className="text-slate-400 text-xs">Saved to device — syncs when you're back in signal</p>
        </div>
          ...
          {(['location', 'photo', 'details'] as Step[]).map((s, i) => (
            <div
              ...
              className={`w-2 h-2 rounded-full transition-colors ${
                step === s ? 'bg-green-400' : step === 'saved' || ['location', 'photo', 'details'].indexOf(step) > i ? 'bg-green-600' : 'bg-slate-600'
              }`}
            />
          ))}
        </div>
      ...
      <div className="flex-1 overflow-y-auto p-4">
        ...
        {step === 'location' && (
          <div className="flex flex-col items-center justify-center h-full gap-6 text-center">
            ...
            {latitude && longitude && (
              <div className="bg-slate-800 rounded-2xl p-4 w-full">
            ...
            {locationError && (
              <div className="bg-red-900/50 border border-red-700 rounded-xl p-4 w-full text-left">
                <p className="text-red-300 text-sm">{locationError}</p>
                <button
                  onClick={getLocation}
                  className="mt-2 text-red-200 text-xs underline"
                >
                  Try again
                </button>
              </div>
            ...
            <button
              ...
              className="w-full py-4 bg-green-600 text-white rounded-2xl font-semibold disabled:bg-slate-700 disabled:text-slate-500 transition-colors"
            >
        ...
        {step === 'photo' && (
          <div className="flex flex-col gap-5">
            <div className="text-center">
              ...
              <p className="text-slate-400 text-sm mt-1">Optional but really helpful — up to 3 photos</p>
            </div>
            ...
            {photoDataUrls.length === 0 ? (
              <button
                ...
                className="w-full h-40 border-2 border-dashed border-slate-600 rounded-2xl flex flex-col items-center justify-center gap-2 text-slate-400 hover:border-green-500 hover:text-green-400 transition-colors"
              >
              ...
              <div className="grid grid-cols-3 gap-2">
                ...
                {photoDataUrls.length < 3 && (
                  <button
                    onClick={() => fileInputRef.current?.click()}
                    className="aspect-square rounded-xl border-2 border-dashed border-slate-600 flex items-center justify-center text-slate-500 hover:border-green-500 transition-colors"
                  >
        ...
        {step === 'details' && (
          <div className="flex flex-col gap-4">
              ...
              <div className="grid grid-cols-2 gap-2">
                <button
                  ...
                  className={`py-3 rounded-xl text-sm font-medium flex items-center justify-center gap-2 transition-colors ${
                    categoryId === 'sheep' ? 'bg-green-600 text-white' : 'bg-slate-700 text-slate-300'
                  }`}
                >
                  🐑 Sheep
                </button>
                {activeCategories.map((cat) => (
                  <button
                    ...
                    className={`py-3 rounded-xl text-sm font-medium flex items-center justify-center gap-2 transition-colors ${
                      categoryId === cat.id ? 'bg-green-600 text-white' : 'bg-slate-700 text-slate-300'
                    }`}
                  >
                    {cat.emoji} {cat.name}
                  </button>
            ...
            {selectedCategory && selectedCategory.conditions.length > 0 && (
              <div>
                <label className="block text-slate-400 text-xs uppercase tracking-wider mb-2">Condition</label>
                <div className="grid grid-cols-2 gap-2">
                  {selectedCategory.conditions.map((c) => (
                    <button
                      ...
                      className={`py-2.5 rounded-xl text-sm font-medium transition-colors ${
                        condition === c ? 'bg-green-600 text-white' : 'bg-slate-700 text-slate-300'
                      }`}
                    >
                      {c}
                    </button>
            ...
            {/* Notes */}
            <div>
              <label className="block text-slate-400 text-xs uppercase tracking-wider mb-2">Notes (optional)</label>
              <textarea
                value={description}
                onChange={(e) => setDescription(e.target.value)}
                placeholder="Any extra details..."
                rows={3}
                className="w-full bg-slate-800 text-white rounded-xl px-4 py-3 text-sm placeholder-slate-500 border border-slate-700 focus:border-green-500 focus:outline-none resize-none"
              />
            </div>
            ...
            <button
              onClick={handleSave}
              disabled={saving}
              className="w-full py-4 bg-green-600 text-white rounded-2xl font-bold text-lg hover:bg-green-700 disabled:bg-slate-700 disabled:text-slate-500 transition-colors mt-2"
            >
        ...
        {step === 'saved' && (
          <div className="flex flex-col items-center justify-center h-full gap-6 text-center">
            <div className="w-20 h-20 rounded-full bg-green-600 flex items-center justify-center text-4xl animate-bounce">
              ✓
            </div>
            <div>
